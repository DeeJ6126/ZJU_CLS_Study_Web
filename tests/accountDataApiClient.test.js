...
test('account data client manages courses, favorites, and notifications below the zjubio prefix', async () => {
  const requests = [];
  ...
  assert.ok(requests.every((request) => request.path.startsWith('/zjubio/api/')));
