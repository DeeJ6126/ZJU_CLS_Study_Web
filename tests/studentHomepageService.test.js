...
test('student homepage application service submits and decides an application', () => {
  ...
  const submitted = submit({ name: 'New A', href: 'https://new.example', note: '请审核' }, { id: 7, nickname: '申请人' });
...
test('student homepage application service submits and admin can list it', () => {
  const { submit, approve, store } = setup();
  const submitted = submit({ name: '张三', href: 'https://zhang.example' }, { id: 'guest-1', nickname: '访客同学' });
  assert.equal(submitted.ok, true);
  ...
  assert.equal(store.listHomepages().some((item) => item.href === 'https://zhang.example'), true);
});
...
test('student homepage application service blocks approving duplicates or missing entries', () => {
  ...
  const first = submit({ name: 'A', href: 'https://a.example' }).application;
  const second = submit({ name: 'A2', href: 'https://a.example' }).application;
  ...
  const third = submit({ name: 'B', href: 'https://b.example' }).application;
