...
- [x] **Step 3: Verify the current production build**
  ...
  `/var/www/html/zjubio`.
  Expected: 378 tests pass before the scheduler addition, Vite exits 0, and project checks report `passed`.
...
- [x] **Step 1: Upgrade only the vulnerable direct dependencies**

  Run `npm.cmd install nodemailer@^9.1.1 sharp@^0.35.4` locally. Do not take
  unrelated major upgrades.

- [x] **Step 2: Verify security and behavior**
...
- Create: `scripts/backup-scheduler.mjs`
- Create: `tests/backupScheduler.test.js`
- Modify: `deploy/README.md`

- [x] **Step 1: Add a failing backup test**
...
- [x] **Step 2: Implement the backup module**
...
- [x] **Step 3: Add a Supervisor-managed daily scheduler**

  The container has neither cron nor crond, so supervise a dedicated Node timer
  instead. It runs as `zjubio_run` at 03:20 local time and survives process
  crashes/container restarts through Supervisor.

- [x] **Step 4: Run and verify one real snapshot**
...
- [x] **Step 3: Reread and restart safely**
...
- [x] **Step 1: Send one real registration code through loopback**

  POST JSON `{"studentId":"3240105782","purpose":"register"}` to
  `http://127.0.0.1:5175/api/auth/email/code` and require HTTP 202. Confirm the
...
- [x] **Step 1: Replace stale deployment statements**
...
- [x] **Step 2: Commit documentation with the corresponding implementation**
